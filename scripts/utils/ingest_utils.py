...
class Ingest:
    ...
    def _apply_cleaners(self, data: pd.DataFrame) -> pd.DataFrame:
            ...
            if file_type == "csv" or file_type == "parquet":
                for batch in reader(file_path):
                    ...
                    is_valid, batch = schema_utils.validate_schema(
                        ...
                        file_path=file_path,
                        strict_column_name_check=False # should allow extra columns
                    )          
                    if not is_valid:
                        continue
                    ...
                    is_valid_post, batch = schema_utils.validate_schema(
                        df=batch, 
                        expected_schema_map=self.final_schema, # final schema na dapat to
                        mismatch_folder=self.mismatch_folder,
                        file_path=file_path + "_POST_TRANSFORM",
                        strict_column_name_check=True # di nag aallow ng extra columns
                    )
                    if not is_valid_post:
                        print(f"Post-transformation failed for  {file_path}.  Not writing to sql.")
                        continue 
                ...
                is_valid_pre, data = schema_utils.validate_schema(
                    df=data, 
                    expected_schema_map=self.expected_schema, 
                    mismatch_folder=self.mismatch_folder,
                    file_path=file_path,
                    strict_column_name_check=False 
                )
                if not is_valid_pre:
                    continue 
                    
                data = self._apply_cleaners(data)    
                is_valid_post, data = schema_utils.validate_schema(
                    df=data, 
                    expected_schema_map=self.final_schema, 
                    mismatch_folder=self.mismatch_folder,
                    file_path=file_path + "_POST_TRANSFORM",
                    strict_column_name_check=True 
                )
                if not is_valid_post:
                    print(f"Post-transformation failed for  {file_path}.  Not writing to sql.")
                    continue
